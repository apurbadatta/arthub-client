...
export default function ArtistProfile({ user }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!userId) {
      ...
      console.log("Current Context User Props:", user);
      console.log("Current Context LocalStorage:", localStorage.getItem("user"));
      return;
    }
    ...
    try {
      ...
      if (res.ok && result.success) {
        // Sync with Better Auth session cookie/JWT on the client side
        try {
        ...
        if (formData.role === "user") {
          toast.info("Role changed to User. Redirecting...");
          setTimeout(() => {
